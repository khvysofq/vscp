...
namespace vscp {
  ...
  Vsid::Vsid(const std::string& vsid_string) {
    if (vsid_string.empty())
      return;

    // First find the slash and slice off that part
    size_t slash = vsid_string.find('/');
    resource_name_ = (slash == std::string::npos ? STR_EMPTY :
      vsid_string.substr(slash + 1));

    // Now look for the node
    size_t at = vsid_string.find('@');
    size_t domain_begin;
    if (at < slash && at != std::string::npos) {
      node_name_ = vsid_string.substr(0, at);
      domain_begin = at + 1;
    }
    else {
      domain_begin = 0;
    ...
    size_t domain_length = (slash == std::string::npos) ?
      (vsid_string.length() - domain_begin) : (slash - domain_begin);
    domain_name_ = vsid_string.substr(domain_begin, domain_length);

    ValidateOrReset();
  }

  Vsid::Vsid(const std::string& node_name,
  ...
  std::string Vsid::Str() const {
    ...
    BOOST_ASSERT(domain_name_ != STR_EMPTY);
    ret += domain_name_;
  ...
  bool Vsid::IsEmpty() const {
    return (node_name_.empty() && domain_name_.empty() &&
      resource_name_.empty());
  }

  bool Vsid::IsValid() const {
    return !domain_name_.empty();
  }

  bool Vsid::IsBare() const {
    if (IsEmpty()) {
      LOG(INFO) << "Warning: Calling IsBare() on the empty vsid.";
      return true;
    }
    return IsValid() && resource_name_.empty();
  }

  bool Vsid::IsFull() const {
    return IsValid() && !resource_name_.empty();
  }

  Vsid Vsid::BareVsid() const {
    if (!IsValid())
      return Vsid();
    if (!IsFull())
      return *this;
    return Vsid(node_name_, domain_name_, STR_EMPTY);
  }

  bool Vsid::BareEquals(const Vsid& other) const {
    return other.node_name_ == node_name_ &&
      other.domain_name_ == domain_name_;
  }

  void Vsid::CopyFrom(const Vsid& vsid) {
    this->node_name_ = vsid.node_name_;
    this->domain_name_ = vsid.domain_name_;
    this->resource_name_ = vsid.resource_name_;
  }

  bool Vsid::operator==(const Vsid& other) const {
    return other.node_name_ == node_name_ &&
      other.domain_name_ == domain_name_ &&
      other.resource_name_ == resource_name_;
  }

  int Vsid::Compare(const Vsid& other) const {
    int compare_result;
  ...
  // Checks and normalizes the node part of a VSID.
  std::string Vsid::PrepNode(const std::string& node, bool* valid) {
    *valid = false;
    std::string result;
  ...
  char Vsid::PrepNodeAscii(char ch, bool* valid) {
    *valid = true;
    switch (ch) {
  ...
  // Checks and normalizes the resource part of a VSID.
  std::string Vsid::PrepResource(const std::string& resource, bool* valid) {
    *valid = false;
    std::string result;
  ...
  char Vsid::PrepResourceAscii(char ch, bool* valid) {
    *valid = true;
    switch (ch) {
    case 0x00: case 0x01: case 0x02: case 0x03: case 0x04: case 0x05:
    case 0x06: case 0x07: case 0x08: case 0x09: case 0x0A: case 0x0B:
  ...
  // Checks and normalizes the domain part of a VSID.
  std::string Vsid::PrepDomain(const std::string& domain, bool* valid) {
    *valid = false;
    std::string result;
  ...
  void Vsid::PrepDomain(const std::string& domain, std::string* buf, bool* valid) {
    *valid = false;
  ...
  char Vsid::PrepDomainLabelAscii(char ch, bool* valid) {
    *valid = true;
