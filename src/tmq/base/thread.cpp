...
#include "tmq/base/criticalsection.h"
...
ThreadManager* ThreadManager::Instance() {
  LIBTMQ_DEFINE_STATIC_LOCAL(ThreadManager, thread_manager, ());
...
void Thread::Send(MessageHandler *phandler, uint32 id, MessageData *pdata) {
  ...
  BOOST_ASSERT(current_thread != NULL);  // AutoThread ensures this
...
bool Thread::WrapCurrentWithThreadManager(ThreadManager* thread_manager) {
  ...
  if (!thread_) {
    LOG(ERROR) << "Unable to get handle to thread.";
    return false;
  }
...
void Thread::UnwrapCurrent() {
  ...
  if (!CloseHandle(thread_)) {
    LOG(ERROR) << "When unwrapping thread, failed to close handle.";
  }
#endif
  started_ = false;
}
...
void ComThread::Run() {
  ...
  if (SUCCEEDED(hr)) {
    ...
    LOG(ERROR) << "CoInitialize failed, hr=" << hr;
  }
}
#endif
