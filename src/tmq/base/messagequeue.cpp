...
void MessageQueueManager::AddInternal(MessageQueue *message_queue) {
  ...
  BOOST_ASSERT(!crit_.CurrentThreadIsOwner());
  CritScope cs(&crit_);
...
void MessageQueueManager::RemoveInternal(MessageQueue *message_queue) {
  BOOST_ASSERT(!crit_.CurrentThreadIsOwner());  // See note above.
...
MessageQueue::MessageQueue(SocketServer* ss)
  ...
  if (!ss_) {
    ...
    //
    default_ss_.reset(new EventWaitServer());
    ss_ = default_ss_.get();
  }
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (pmsg->ts_sensitive) {
        ...
        if (delay > 0) {
          LOG(WARNING) << "id: " << pmsg->message_id << "  delay: "
                            << (delay + kMaxMsgLatency) << "ms";
        }
      }
      // If this was a dispose message, delete it and skip it.
      if (MQID_DISPOSE == pmsg->message_id) {
        BOOST_ASSERT(NULL == pmsg->phandler);
        delete pmsg->pdata;
        *pmsg = Message();
        continue;
      }
...
void MessageQueue::DoDelayPost(int cmsDelay, uint32 tstamp,
  ...
  BOOST_VERIFY(0 != ++dmsgq_next_num_);
  ss_->WakeUp();
}
...
void MessageQueue::Clear(MessageHandler *phandler, uint32 id, MessageList* removed) {
